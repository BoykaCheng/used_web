...
/**
 * Created by Chenge on 2019.11.13 17:56
 */
@Component
public class RedisUtil {
    ...
    /**
     * 插入zset  如果元素存在，会用新的score来替换原来的，返回0；如果元素不存在，则会会新增一个
     * @param key
     * @param value
     * @param score
     * @param time
     * @return
     */
    public Boolean zSet(String key, String value, double score,long time){
  ...
  /*  public void addToListRight(String listKey, Status.ExpireEnum expireEnum, Object... values) {
     ...
     *
     * XXX为value的类型，前者获取一个operator，但是没有指定操作的对象（key），
     * 可以在一个连接（事务）内操作多个key以及对应的value；
     *
     * 后者获取了一个指定操作对象（key）的operator，
     * 在一个连接（事务）内只能操作这个key对应的value。
     * ————————————————
